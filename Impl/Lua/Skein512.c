#include <PPQ/lua/skein512.h>

#define MT_		PPQ_LUA_SKEIN512_MT
#define NEW_(L)		PPQ_LUA_SKEIN512_NEW(L)
#define CHECK_(L, idx)	PPQ_LUA_SKEIN512_CHECK(L, idx)
#define TEST_(L, idx)	PPQ_LUA_SKEIN512_TEST(L, idx)
#define NULL_		PPQ_LUA_SKEIN512_NULL_LITERAL

typedef PPQ_Lua_Skein512 Skein512_t;
...
static int skein512_hash(lua_State* L) 
{
	...
	PPQ_Skein512_hash(sk, bytes_out, bytes_in, num_bytes_in, num_bytes_out);
	return 0;
}
...
static int skein512_hash_native (lua_State* L)
{
	...
	if (!(bytes_in = (uint8_t*)lua_touserdata(L, 3)))
		return luaL_error(L, "Invalid ptr arg %d.", 2);
	num_bytes_in = (uint64_t)luaL_checkinteger(L, 4);
	PPQ_Skein512_hash_native(sk, bytes_out, bytes_in, num_bytes_in);
	return 0;
}
...
static int skein512_mac(lua_State* L)
{
	...
	PPQ_Skein512_mac(sk, bytes_out, bytes_in, key_in, num_bytes_in, num_bytes_out);
	return 0;
}
...
int luaopen_PPQ_Skein512 (lua_State* L)
{
