...
SKC_INLINE void Skc_rand_uint16(Skc_CSPRNG* csprng, uint16_t* buffer) IMPL_

/* Store a pseudorandom uint32_t at @buffer. */
SKC_INLINE void Skc_rand_uint32(Skc_CSPRNG* csprng, uint32_t* buffer) IMPL_

/* Store a pseudorandom uint64_t at @buffer. */
SKC_INLINE void Skc_rand_uint64(Skc_CSPRNG* csprng, uint64_t* buffer) IMPL_
...
SKC_INLINE uint64_t Skc_rand_range(
 Skc_CSPRNG*    csprng,
 const uint64_t min_val, /* Generate a natural number at minimum @min_val. */
 const uint64_t max_val) /* Generate a natural number at most @max_val. */
{
