...
Skc_Dragonfly_V1_encrypt(
 Skc_Dragonfly_V1_Encrypt* const R_ ctx,
 Base_MMap*  const R_               input_mmap,
 Base_MMap*  const R_               output_mmap,
 const char* const R_               output_filepath)
{
  ...
  memcpy(out, SKC_DRAGONFLY_V1_ID, SKC_DRAGONFLY_V1_ID_NBYTES);
  out += SKC_DRAGONFLY_V1_ID_NBYTES;
...
void Skc_Dragonfly_V1_decrypt (Skc_Dragonfly_V1_Decrypt* const R_ ctx,
                               Base_MMap*   const R_              input_mmap,
                               Base_MMap*   const R_              output_mmap,
                               const char* const R_               output_filepath)
{
...
#define MIN_POSSIBLE_FILE_SIZE_ (SKC_DRAGONFLY_V1_VISIBLE_METADATA_BYTES + 1)
        ...
        uint8_t  header_id      [SKC_DRAGONFLY_V1_ID_NBYTES];
        uint8_t  g_low;
        uint8_t  g_high;
        uint8_t  lambda;
        uint8_t  use_phi;
        ...
        if (memcmp(header_id, SKC_DRAGONFLY_V1_ID, SKC_DRAGONFLY_V1_ID_NBYTES)) {
...
void Skc_Dragonfly_V1_dump_header (Base_MMap* const R_ mem_map, const char* const R_ filepath) {
#define MIN_SIZE_ (SKC_DRAGONFLY_V1_VISIBLE_METADATA_BYTES + 1)
        ...
        uint8_t  id [SKC_DRAGONFLY_V1_ID_NBYTES];
        uint64_t total_size;
        uint8_t  g_low;
        uint8_t  g_high;
        uint8_t  lambda;
